...
print_node(ast_node *node){
    ...
    switch (tag) {
        ...
        case AST_COMPILE_UNIT_MEMBER_LIST:
            ...
            break;
        case AST_RETURN_STATEMENT:
            fprintf(stdout, "AST_RETURN_STATEMENT\n");
            if (((ast_return_statement*)node)->expression)
                print_node(((ast_return_statement*)node)->expression);
            break;
        case AST_METHOD:
...
create_return_statement(ast_node* expression) 
{
    struct ast_return_statement * ret;
    ret = alloc_node(AST_RETURN_STATEMENT);
